  ...
  return (
    <div className="flex h-full bg-background">
      <div className="hidden h-full w-80 shrink-0 border-r border-border bg-card/60 lg:flex">
        <CatalogExplorer
          mode="select"
          activeCatalog={catalogParam || undefined}
          activeNamespace={namespaceParam || undefined}
          activeTable={tableParam || undefined}
          onSelectTable={({ catalog, namespace, table }) => {
            if (!catalog || !namespace || !table) return
            setQuery(
              `SELECT * FROM \`${catalog}\`.\`${namespace}\`.\`${table}\` LIMIT 100`
            )
            toast({
