...
export const propertyDescriptions: Record<string, string> = {
    // Commit properties
    "commit.retry.num-retries": "Number of times to retry a commit before failing",
    "commit.retry.min-wait-ms": "Minimum time in milliseconds to wait before retrying a commit",
    "commit.retry.max-wait-ms": "Maximum time in milliseconds to wait before retrying a commit",
    "commit.retry.total-timeout-ms": "Total retry timeout period in milliseconds for a commit",
    ...
    "commit.status-check.min-wait-ms": "Minimum time in milliseconds to wait before retrying a status-check",
    "commit.status-check.max-wait-ms": "Maximum time in milliseconds to wait before retrying a status-check",
    ...
    // Write properties
    "write.format.default": "Default file format for the table; parquet, avro, or orc",
    ...
    "write.parquet.compression-codec": "Parquet compression codec: zstd, brotli, lz4, gzip, snappy, uncompressed",
    "write.parquet.compression-level": "Parquet compression level",
    "write.parquet.bloom-filter-enabled.column": "Hint to parquet to write a bloom filter for the column",
    "write.parquet.bloom-filter-max-bytes": "The maximum number of bytes for a bloom filter bitset",
    "write.parquet.bloom-filter-fpp.column": "The false positive probability for a bloom filter (must > 0.0 and < 1.0)",
    "write.avro.compression-codec": "Avro compression codec: gzip(deflate with 9 level), zstd, snappy, uncompressed",
    ...
    "write.orc.block-size-bytes": "Define the default file system block size for ORC files",
    "write.orc.compression-codec": "ORC compression codec: zstd, lz4, lzo, zlib, snappy, none",
    "write.orc.compression-strategy": "ORC compression strategy: speed, compression",
    "write.orc.bloom.filter.columns": "Comma separated list of column names for which a Bloom filter must be created",
    "write.orc.bloom.filter.fpp": "False positive probability for Bloom filter (must > 0.0 and < 1.0)",
    "write.location-provider.impl": "Optional custom implementation for LocationProvider",
    "write.metadata.compression-codec": "Metadata compression codec; none or gzip",
    ...
    "write.target-file-size-bytes": "Controls the size of files generated to target about this many bytes",
    "write.delete.target-file-size-bytes": "Controls the size of delete files generated to target about this many bytes",
    "write.distribution-mode": "Defines distribution of write data: none, hash, or range",
    ...
    "write.object-storage.partitioned-paths": "Includes the partition values in the file path",
    "write.data.path": "Base location for data files",
    "write.metadata.path": "Base location for metadata files",
    "write.delete.mode": "Mode used for delete commands: copy-on-write or merge-on-read (v2 only)",
    "write.delete.isolation-level": "Isolation level for delete commands: serializable or snapshot",
    "write.update.mode": "Mode used for update commands: copy-on-write or merge-on-read (v2 only)",
    "write.update.isolation-level": "Isolation level for update commands: serializable or snapshot",
    "write.merge.mode": "Mode used for merge commands: copy-on-write or merge-on-read (v2 only)",
    "write.merge.isolation-level": "Isolation level for merge commands: serializable or snapshot",

    // Read properties
    "read.split.target-size": "Target size when combining data input splits",
    "read.split.metadata-target-size": "Target size when combining metadata input splits",
    "read.split.planning-lookback": "Number of bins to consider when combining input splits",
    "read.split.open-file-cost": "The estimated cost to open a file, used as a minimum weight when combining splits",
    "read.parquet.vectorization.enabled": "Controls whether Parquet vectorized reads are used",
    "read.parquet.vectorization.batch-size": "The batch size for parquet vectorized reads",
    "read.orc.vectorization.enabled": "Controls whether orc vectorized reads are used",
    "read.orc.vectorization.batch-size": "The batch size for orc vectorized reads",
}
