...
/**
 * Utility class for handling cron expressions. Supports standard cron format: second minute hour
...
public class CronUtil {
    ...
    public static LocalDateTime getNextExecutionTime(String cronExpression, LocalDateTime from) {
        ...
        throw new IllegalArgumentException(
                "Unable to find next execution time for: " + cronExpression);
    }
    ...
    private static LocalDateTime getNextIncrement(LocalDateTime current, String[] cronParts) {
        ...
        if (!cronParts[2].equals("*")
                && !cronParts[2].contains("/")
                && !cronParts[2].contains("-")
                && !cronParts[1].equals("*")
                && !cronParts[1].contains("/")
                && !cronParts[1].contains("-")
                && !cronParts[0].equals("*")
                && !cronParts[0].contains("/")
                && !cronParts[0].contains("-")
                && cronParts[3].equals("*")
                && cronParts[4].equals("*")
                && cronParts[5].equals("*")) {
            ...
            LocalDateTime todayTarget =
                    current.toLocalDate().atTime(targetHour, targetMinute, targetSecond);
        ...
        if (!cronParts[2].equals("*")
            ...
            LocalDateTime todayTarget =
                    current.toLocalDate().atTime(targetHour, targetMinute, targetSecond);
            if (todayTarget.isAfter(current)
                    && matchesField(current.getDayOfWeek().getValue() % 7, cronParts[5], 0, 6)) {
                return todayTarget;
            }
            ...
            for (int i = 1; i <= 7; i++) {
                LocalDateTime nextDay =
                        current.plusDays(i)
                                .toLocalDate()
                                .atTime(targetHour, targetMinute, targetSecond);
        ...
        if (!cronParts[1].equals("*")
            ...
            LocalDateTime thisHourTarget =
                    current.withMinute(targetMinute).withSecond(targetSecond);
