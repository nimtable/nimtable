    ...
    private void initializeDuckDB(Connection conn, Config config) throws Exception {
        try (Statement stmt = conn.createStatement()) {
            ...
            for (Config.Catalog catalog : config.catalogs()) {
                ...
                String s3SecretName = "s3-secret-" + catalog.name();
                String s3Region = catalog.properties().getOrDefault("s3.region", "us-east-1");
                String s3EndpointUrl = catalog.properties().getOrDefault("s3.endpoint", "https://s3.amazonaws.com");
                ...
                String icebergSecretName = "iceberg-secret-" + catalog.name();
                String icebergEndpoint = String.format("http://%s:%d/api/catalog/%s", config.server().host(), config.server().port(), catalog.name());
                query = String.format(
                ...
                // Attach catalog
                stmt.execute(String.format("ATTACH '' AS \"%s\" (TYPE ICEBERG)", catalog.name()));
            }
        }
    }
