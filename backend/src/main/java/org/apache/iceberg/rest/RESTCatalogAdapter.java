...
public class RESTCatalogAdapter extends BaseHTTPClient {
    ...
    enum Route {
        ...
        LOAD_NAMESPACE(
                HTTPMethod.GET, ResourcePaths.V1_NAMESPACE, null, GetNamespaceResponse.class),
        ...
        RENAME_TABLE(
                HTTPMethod.POST, ResourcePaths.V1_TABLE_RENAME, RenameTableRequest.class, null),
        REPORT_METRICS(
                            ...
                            .allMatch(
                                    requirement ->
                                            requirement
                                                    .getValue()
                                                    .equalsIgnoreCase(
                                                            requestPath.get(requirement.getKey())));
        }
    ...
    public <T extends RESTResponse> T handleRequest(
        ...
        switch (route) {
            ...
            case NAMESPACE_EXISTS:
                if (asNamespaceCatalog != null) {
                    CatalogHandlers.namespaceExists(
                            asNamespaceCatalog, namespaceFromPathVars(vars));
                    return null;
                }
                break;
            ...
            case LIST_TABLES:
                {
                    ...
                    if (pageSize != null) {
                        return castResponse(
                                responseType,
                                CatalogHandlers.listTables(
                                        catalog, namespace, pageToken, pageSize));
                    } else {
                        return castResponse(
            ...
            case UPDATE_TABLE:
                {
                    ...
                    return castResponse(
                            responseType, CatalogHandlers.updateTable(catalog, ident, request));
                }
            ...
            case LIST_VIEWS:
                {
                    if (null != asViewCatalog) {
                        ...
                        if (pageSize != null) {
                            return castResponse(
                                    responseType,
                                    CatalogHandlers.listViews(
                                            asViewCatalog, namespace, pageToken, pageSize));
                        } else {
                            return castResponse(
                                    responseType,
            ...
            case LOAD_VIEW:
                {
                    if (null != asViewCatalog) {
                        ...
                        return castResponse(
                                responseType, CatalogHandlers.loadView(asViewCatalog, ident));
                    }
                    break;
                }
     ...
     * does not do any other conflict detection. Therefore, it does not guarantee true transactional
            ...
            if (table instanceof BaseTable) {
                Transaction transaction =
                        Transactions.newTransaction(
                                tableChange.identifier().toString(),
        ...
        if (routeAndVars != null) {
            try {
                ...
                return handleRequest(
                        routeAndVars.first(), vars.build(), request.body(), responseType);
    ...
    public void close() throws IOException {
        // The calling test is responsible for closing the underlying catalog backing this REST
        // catalog
        // so that the underlying backend catalog is not closed and reopened during the REST
        // catalog's
    ...
    private static class BadResponseType extends RuntimeException {
        private BadResponseType(Class<?> responseType, Object response) {
            super(
                    String.format(
                            "Invalid response object, not a %s: %s",
                            responseType.getName(), response));
        }
    }
    ...
    private static class BadRequestType extends RuntimeException {
        private BadRequestType(Class<?> requestType, Object request) {
            super(
                    String.format(
                            "Invalid request object, not a %s: %s",
                            requestType.getName(), request));
        }
    }
